...
.vote-app .toolbar { display:flex; gap:10px; align-items:center; flex-wrap:wrap; }
.vote-app .btn.good { background: #16a34a; }
.vote-app .btn.warn { background: #f59e0b; }
.vote-app .btn:disabled { opacity: .6; cursor: not-allowed; }
.vote-app .progress { font-size:14px; color: var(--muted); }
.vote-app .region {
  margin: 18px 6px;
...
.vote-app .region h2 { margin:4px 2px 10px; font-weight:800; font-size: clamp(18px,2.2vw,22px); }
.vote-app .group { padding:10px 8px 12px; border-radius:14px; transition:background .25s ease; }
.vote-app .group:hover { background: rgba(255,255,255,.45); }
.vote-app .art {
  display:block;
  width:min(420px,100%);
  height:auto;
...
.vote-app .options { display:flex; flex-wrap:wrap; gap:10px; }
.vote-app .option {
  display:inline-flex;
  align-items:center;
...
.vote-app .option:hover { transform: translateY(-1px); }
.vote-app .option[aria-checked="true"] {
  background:#b6f1ff;
  border-color:#9ae8ff;
  box-shadow:0 10px 26px rgba(34,211,238,.28), inset 0 1px 0 rgba(255,255,255,.9);
}
.vote-app .option:focus-visible {
  box-shadow:0 0 0 3px rgba(34,211,238,.5), 0 6px 16px rgba(34,211,238,.25);
}
.vote-app .summary {
  position:sticky;
  bottom:10px;
...
.vote-app .mini { font-size:12px; color:var(--muted); }
.vote-app details.howto { margin:10px 6px 0; }
.vote-app details.howto summary { cursor:pointer; font-weight:700; }
@media (prefers-reduced-motion: reduce) {
  .vote-app .option { transition:none; }
}
</style>
<div class="vote-app" id="app" aria-live="polite">
    <header>
