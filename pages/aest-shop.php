...
$inventory = array_values(array_filter(shop_inventory($shopId), static function (array $item) use ($keywords): bool {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && input_string($_POST['action'] ?? '', 20) === 'checkout') {
    ...
    echo json_encode([
        'ok' => true,
        'message' => 'Fangroot pack order prepared. Please proceed to the counter to finalize payment.',
...
<section class="pizza-shop">
  <header class="pizza-shop__header">
    <h1><?= htmlspecialchars($shop['shop_name'] ?? 'Aeonstep Provisioners') ?></h1>
    <p class="muted">A Fangroot Canyon outfitter carrying only Prehistoric, Neogenic, and Aeonstepian-tagged finds.</p>
  </header>
  <div class="pizza-shop__layout">
    <div class="card glass pizza-shop__menu">
      <h2>Fangroot Catalog</h2>
      <?php if (!$inventory): ?>
      <p class="muted">No matching finds are stocked right now. Check back after the next trade caravan.</p>
      <?php else: ?>
      <div class="pizza-menu-grid" role="list" aria-label="Available Fangroot items">
    ...
    <aside class="card glass pizza-cart" aria-label="Shopping cart">
      <div class="pizza-cart__header">
        <h2>Your Order</h2>
        <p class="muted">Adjust quantities or remove finds before you submit.</p>
      </div>
      <div class="pizza-cart__body">
        <p class="pizza-cart__empty" id="pizza-cart-empty">Select an item to begin building your order.</p>
