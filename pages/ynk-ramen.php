...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'checkout') {
    ...
    echo json_encode([
        'ok' => true,
        'message' => 'Your bowls are lining up. Please head to the counter to settle the tab.',
...
<section class="pizza-shop">
  <header class="pizza-shop__header">
    <h1><?= htmlspecialchars($shop['shop_name'] ?? 'Yumenoki Ramen') ?></h1>
    <p class="muted">Steam, cedar, and quiet radio crackle. Broth simmers from dawn, noodles roll by hand, and the cook nods you toward an empty stool.</p>
  </header>
  <div class="pizza-shop__layout">
    <div class="card glass pizza-shop__menu">
      <h2>Menu</h2>
      <?php if (!$inventory): ?>
      <p class="muted">The pots are rinsed clean for now. Check back after the next simmer.</p>
      <?php else: ?>
      <div class="pizza-menu-grid" role="list" aria-label="Available bowls">
    ...
    <aside class="card glass pizza-cart" aria-label="Shopping cart">
      <div class="pizza-cart__header">
        <h2>Your Order</h2>
        <p class="muted">Adjust bowls, sides, or tea before you slide the tray across.</p>
      </div>
      <div class="pizza-cart__body">
        <p class="pizza-cart__empty" id="pizza-cart-empty">Tap a bowl or side to start your order.</p>
...
(function() {
  ...
  cartBuyBtn.addEventListener('click', async () => {
    ...
    cartBuyBtn.disabled = true;
    setStatus('Calling your order to the counter…');
    ...
    try {
      ...
      let message = 'The order slip smudged. Please try again.';
