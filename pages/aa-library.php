...
$shopId = 5;
$shop = shop_get($shopId) ?? ['shop_id' => $shopId, 'shop_name' => 'Heliadora Civic Library'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && input_string($_POST['action'] ?? '', 20) === 'checkout') {
    ...
    if (!$cartPayload) {
        echo json_encode(shop_error_payload('Your satchel is empty.', [
    ...
    foreach ($cartPayload as $entry) {
        ...
        if (!isset($inventoryById[$itemId])) {
            echo json_encode(shop_error_payload('One of the tomes is no longer available here.', [
                'item_id' => $itemId,
                'shop_id' => $shopId,
            ]));
            exit;
        ...
        if ($maxStock !== null && $orderItems[$itemId]['quantity'] > $maxStock) {
            echo json_encode(shop_error_payload(
                sprintf('You can only request up to %d × %s.', $maxStock, $item['name']),
                [
                    'item_id' => $itemId,
    ...
    try {
        ...
        $message = $err instanceof RuntimeException
            ? $err->getMessage()
            : 'We could not finalize your request. Please try again.';
    ...
    echo json_encode([
        'ok' => true,
        'message' => 'Your reading room docket has been prepared.',
...
<section class="pizza-shop">
  ...
  <div class="pizza-shop__layout">
    <div class="card glass pizza-shop__menu">
      <h2>Collection</h2>
      <?php if (!$inventory): ?>
      <p class="muted">The archivists are reshelving right now. Please check back shortly.</p>
      <?php else: ?>
      <div class="pizza-menu-grid" role="list" aria-label="Available tomes">
          ...
          <figure>
            ...
            <figcaption>
              ...
              <span class="pizza-item-stock<?= $isSoldOut ? ' sold-out' : '' ?>">
                <?php if ($isSoldOut): ?>Unavailable<?php else: ?>Stock: <?= $stockLabel ?><?php endif; ?>
              </span>
              <?php else: ?>
    ...
    <aside class="card glass pizza-cart" aria-label="Borrowing cart">
      <div class="pizza-cart__header">
        <h2>Your Requests</h2>
        <p class="muted">Adjust quantities or remove volumes before you submit.</p>
      </div>
      <div class="pizza-cart__body">
        <p class="pizza-cart__empty" id="pizza-cart-empty">Select a volume to begin building your request.</p>
      ...
      <footer class="pizza-cart__footer">
        ...
        <div class="pizza-cart__actions">
          ...
          <button type="button" class="btn" id="pizza-cart-buy" disabled>Submit</button>
        </div>
...
(function() {
  ...
  function updateMenuButtonStock(itemId, stock) {
    ...
    if (stockLabel) {
      if (isSoldOut) {
        stockLabel.textContent = 'Unavailable';
  ...
  menuButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (btn.disabled) {
        ...
        if (item) {
          setStatus(`${item.name} is unavailable right now.`);
        }
        return;
      }
  ...
  cartBuyBtn.addEventListener('click', async () => {
    ...
    cartBuyBtn.disabled = true;
    setStatus('Submitting your request to the archivists…');
    ...
    try {
      ...
      if (!data.ok) {
        ...
        setStatus(data.error || 'Could not process your request.', data.context || null);
        cartBuyBtn.disabled = false;
        return;
      }
      ...
      let message = 'A cataloguing gremlin intercepted the request. Please try again.';
