...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && input_string($_POST['action'] ?? '', 20) === 'checkout') {
    ...
    if (!$inventoryById) {
        echo json_encode(shop_error_payload('This kiosk has no stock available.', [
            'shop_id' => $shopId,
        ]));
        exit;
    }
    ...
    if (!$cartPayload) {
        echo json_encode(shop_error_payload('Your counter tray is empty.', [
    ...
    foreach ($cartPayload as $entry) {
        ...
        if (!isset($inventoryById[$itemId])) {
            echo json_encode(shop_error_payload('One of the kiosk items is no longer sold here.', [
                'item_id' => $itemId,
                'shop_id' => $shopId,
            ]));
            exit;
        ...
        if ($maxStock !== null && $orderItems[$itemId]['quantity'] > $maxStock) {
            echo json_encode(shop_error_payload(
                sprintf('You can only grab up to %d x %s.', $maxStock, $item['name']),
                [
                    'item_id' => $itemId,
    ...
    try {
        ...
        $message = $err instanceof RuntimeException
            ? $err->getMessage()
            : 'We could not settle the kiosk tab. Please try again.';
    ...
    echo json_encode([
        'ok' => true,
        'message' => 'Karl bags everything up and slides it across the counter.',
...
<section class="pizza-shop">
  <header class="pizza-shop__header">
    <h1><?= htmlspecialchars($shop['shop_name'] ?? "Karl's grosser Kiosk") ?></h1>
    <p class="muted">A bright little Rheinland kiosk tucked beside the stonebridge foot traffic, with Scratch tickets hanging by the till, small baggies of sweets stacked in jars, and quick comforts for anyone crossing Rheingard in a hurry.</p>
  </header>
  <div class="pizza-shop__layout">
    <div class="card glass pizza-shop__menu">
      <h2>Counter Stock</h2>
      <?php if (!$inventory): ?>
      <p class="muted">Karl is restocking the shelves right now. Check back after the next bell.</p>
      <?php else: ?>
      <div class="pizza-menu-grid" role="list" aria-label="Available kiosk items">
        <?php foreach ($inventory as $item): ?>
        <?php
          $stock = $item['stock'];
          $priceDisplay = number_format($item['price'], 2);
          $stockLabel = $stock === null ? 'plentiful' : (string) $stock;
          ...
          <figure>
            ...
            <figcaption>
              ...
              <span class="pizza-item-stock<?= $isSoldOut ? ' sold-out' : '' ?>">
                <?php if ($isSoldOut): ?>Sold out<?php else: ?>Stock: <?= htmlspecialchars($stockLabel) ?><?php endif; ?>
              </span>
              <?php else: ?>
    ...
    <aside class="card glass pizza-cart" aria-label="Shopping cart">
      <div class="pizza-cart__header">
        <h2>Your Tray</h2>
        <p class="muted">Adjust your snacks and supplies before you pay Karl.</p>
      </div>
      <div class="pizza-cart__body">
        <p class="pizza-cart__empty" id="pizza-cart-empty">Tap an item to start building your tray.</p>
...
(function() {
  ...
  cartBuyBtn.addEventListener('click', async () => {
    ...
    cartBuyBtn.disabled = true;
    setStatus('Karl is ringing up your tray...');
    ...
    try {
      ...
      let message = 'The till jammed. Please try again.';
