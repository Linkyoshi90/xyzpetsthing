...
$shopId = 0; // TODO: set Emberfen grill/diner shop id
$shop = shop_get($shopId) ?? ['shop_id' => $shopId, 'shop_name' => 'Emberfen Grillhouse'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && input_string($_POST['action'] ?? '', 20) === 'checkout') {
    ...
    echo json_encode([
        'ok' => true,
        'message' => 'Order fired up! Please proceed to the counter to finalize payment.',
...
<section class="pizza-shop">
  <header class="pizza-shop__header">
    <h1><?= htmlspecialchars($shop['shop_name'] ?? 'Emberfen Grillhouse') ?></h1>
    <p class="muted">A fen-edge grill and diner serving seared catches, smoked roots, and hot-plate comfort meals.</p>
  </header>
  <div class="pizza-shop__layout">
    <div class="card glass pizza-shop__menu">
      <h2>Grill Menu</h2>
      <?php if (!$inventory): ?>
      <p class="muted">The griddles are cooling right now. Please check back soon.</p>
      <?php else: ?>
      <div class="pizza-menu-grid" role="list" aria-label="Available grill and diner dishes">
    ...
    <aside class="card glass pizza-cart" aria-label="Shopping cart">
      <div class="pizza-cart__header">
        <h2>Your Order</h2>
        <p class="muted">Adjust quantities or remove plates before you submit.</p>
      </div>
      <div class="pizza-cart__body">
        <p class="pizza-cart__empty" id="pizza-cart-empty">Select a plate to begin building your order.</p>
...
(function() {
  ...
  function updateGrill MenuButtonStock(itemId, stock) {
    ...
    try {
      ...
      if (data.stock && typeof data.stock === 'object') {
        Object.entries(data.stock).forEach(([itemId, stockValue]) => {
          updateGrill MenuButtonStock(itemId, stockValue);
        });
      }
